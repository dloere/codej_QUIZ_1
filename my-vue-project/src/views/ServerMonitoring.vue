<template>
    <div>
        ...
        <div style="height: 200px; border:1px solid; overflow: auto;">
            <h1>도움말</h1>
            <div>사용용도 : 등록한 서버를 주기적으로 모니터링</div> 
            <div></div> 
        </div>

        <div style="margin: 10px">
            <button @click="startMonitoring">{{ intervalId ? '감시해제' : '감시시작' }}</button>
            반복주기(1000 = 1초) : <input v-model="delay" placeholder="반복 주기 입력" />
        </div>
        ...
        <div style="display:flex; width: 100%">
            <div style="width: 500px">
                ...
                <!-- 결과 표시 -->
                <div v-if="pingResult">
                <div v-html="pingResult" style="white-space: pre-line;"></div>
            </div>
        </div>
...
export default {
    ...
    methods: {
        ...
        updateLocalStorage() {
            // 이력을 로컬 스토리지에 저장
            localStorage.setItem('monitoringServer', JSON.stringify(this.history));
        },

        // 로컬 스토리지에서 이력 불러오기
        loadHistoryFromLocalStorage() {
            const savedHistory = JSON.parse(localStorage.getItem('monitoringServer') || '[]');
